...
contract RewardsManagerFixed is ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => mapping(address => uint256)))
        public points; // Calculate points per each epoch points[epochId][vaultAddress][userAddress]
    mapping(uint256 => mapping(address => mapping(address => mapping(address => uint256))))
        public pointsWithdrawn; // Given point for epoch how many where withdrawn by user? pointsWithdrawn[epochId][vaultAddress][userAddress][rewardToken]
    ...
    mapping(uint256 => mapping(address => mapping(address => uint256)))
        public lastUserAccrueTimestamp; // Last timestamp in we accrued user to calculate rewards in epochs without interaction lastUserAccrueTimestampepochId][vaultAddress][userAddress]
    ...
    mapping(uint256 => mapping(address => mapping(address => uint256)))
        public shares; // Calculate points per each epoch shares[epochId][vaultAddress][userAddress]
    ...
    mapping(uint256 => mapping(address => mapping(address => uint256)))
        public rewards; // rewards[epochId][vaultAddress][tokenAddress] = AMOUNT
    ...
    function accrueVault(uint256 epochId, address vault) public {
        ...
        (uint256 supply, bool shouldUpdate) = getTotalSupplyAtEpoch(
            epochId,
            vault
        );
    ...
    function getVaultTimeLeftToAccrue(uint256 epochId, address vault)
        public
        view
        returns (uint256)
    {
    ...
    function getTotalSupplyAtEpoch(uint256 epochId, address vault)
        public
        view
    ...
    function claimRewards(
        ...
        require(
            epochLength == vaultLength &&
                epochLength == tokensLength &&
                epochLength == usersLength,
            "Length mismatch"
        );
    ...
    function claimBulkTokensOverMultipleEpochs(
        ...
        for (uint256 epochId = epochStart; epochId <= epochEnd; ++epochId) {
            ...
            for (uint256 i = 0; i < tokensLength; ++i) {
                ...
                uint256 totalAdditionalReward = rewards[epochId][vault][
                    tokens[i]
                ];
                uint256 tokensForUser = (totalAdditionalReward * ratioPoints) /
                    PRECISION;
    ...
    function claimBulkTokensOverMultipleEpochsOptimized(
        uint256 epochStart,
        uint256 epochEnd,
        address vault,
        address[] calldata tokens
    ...
    function accrueUser(
        ...
        (uint256 currentBalance, bool shouldUpdate) = getBalanceAtEpoch(
            epochId,
            vault,
            user
        );
        ...
        uint256 timeInEpochSinceLastAccrue = getUserTimeLeftToAccrue(
            epochId,
            vault,
            user
        );
    ...
    function getUserTimeLeftToAccrue(
        uint256 epochId,
        address vault,
        address user
    ) public view returns (uint256) {
        uint256 lastBalanceChangeTime = lastUserAccrueTimestamp[epochId][vault][
            user
        ];
        Epoch memory epochData = epochs[epochId];
